...
export function tableState(room,viewerId) {
  ...
  for(const s of slots){s.canPlace=!!(active&&s.available&&(funds>=s.cost||funds<minCost));s.detail=s.label+' · '+(s.owner?s.owner.name+' 已占用':s.cost?'费用 '+s.cost+'₱':'立即领取 10₱')+(s.reward?' · 回报 '+s.reward+'₱':'');}
