(function () {
  ...
  function observeTurn(nextRoom) {
    ...
    if (observedTurnOwnerId === API.playerId() && previousOwnerId !== observedTurnOwnerId) UI.alertTurn(nextRoom);
  }
  ...
  document.getElementById('create-room').addEventListener('click', async function () {
    ...
    setLobbySubmitting(true);
    try {
      var result = await API.createRoom(document.getElementById('room-name').value.trim(), Number(document.getElementById('room-max').value), Number(document.getElementById('decision-seconds').value), document.getElementById('game-mode').value);
  ...
  document.getElementById('game-mode').addEventListener('change', function (event) {
    var max = document.getElementById('room-max');
