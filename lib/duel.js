...
function step(room) {
  ...
  if (room.players.some(p => !p.dummy && Date.now() - p.lastSeen > 1800)) { room.paused = true; return; }
  ...
  room.fighters.forEach((f, i) => {
    ...
    if (f.attack) {
      ...
      if (!a.hit && a.age >= a.active) { a.hit = true; if (Math.abs(f.x-enemy.x) < a.reach && Math.abs(f.y-enemy.y)<100 && !enemy.inv && !['floor','rise'].includes(enemy.down)) hits.push({i, a}); }
...
function install(server) {
  ...
  const view=r=>({code:r.code,players:r.players.map(p=>({name:p.name,type:p.type,ready:p.ready,dummy:!!p.dummy,online:!!p.dummy||Date.now()-p.lastSeen<1800})),fighters:r.fighters,phase:r.phase,time:r.time,countdown:r.countdown,winner:r.winner,effects:r.effects,paused:r.paused,tick:r.tick,hitstop:r.hitstop});
  ...
  return async function handle(req,res,url) {
    try {
      if(req.method==='GET'&&url.pathname==='/api/duel/events') {
        const r=rooms.get(url.searchParams.get('code')); const p=r?.players.find(p=>!p.dummy&&p.token===url.searchParams.get('token')); if(!p)return reply(res,403,{error:'房间凭证失效，请重新加入'});
      ...
      r=rooms.get(String(b.code));const i=r?.players.findIndex(p=>!p.dummy&&p.token===b.token);if(!r||i<0)throw Error('房间凭证失效，请重新加入');const p=r.players[i];p.lastSeen=Date.now();r.touched=Date.now();
      if(action==='input'){const input={};for(const k of ['left','right','jump','dash','block','light','skill','special','ultimate'])input[k]=b.input?.[k]===true;const f=r.fighters[i]; f.pulses ||= {}; for(const k of ['jump','dash','light','skill','special','ultimate']) if(input[k]&&!f.input[k])f.pulses[k]=true; f.input=input;}
      else if(action==='add-dummy'||action==='remove-dummy') {
        ...
        if(action==='add-dummy') {
          ...
          r.players.push({name:'测试假人',type,dummy:true,ready:true});
          r.fighters.push(fighter(type,1));
          if(p.ready)reset(r);
        } else {
