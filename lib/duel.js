...
function step(room) {
  ...
  if (room.fighters.some(f=>f.hp<=0) || room.time<=0) { room.phase='over'; const [a,b]=room.fighters; room.winner=a.hp===b.hp?-1:a.hp>b.hp?0:1; room.players.forEach(p=>p.ready=!!p.dummy); }
}
function install(server) {
  ...
  const view=r=>({code:r.code,players:r.players.map(p=>({name:p.name,type:p.type,ready:p.ready,dummy:!!p.dummy,online:!!p.dummy||Date.now()-p.lastSeen<1800})),fighters:r.fighters,phase:r.phase,time:r.time,countdown:r.countdown,winner:r.winner,effects:r.effects,paused:r.paused,tick:r.tick});
